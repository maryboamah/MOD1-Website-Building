...
window.addEventListener("load", () => {
  ...
  if (userSearchValueup != "" && userSearchSelectionup != "") {
    ...
    document.getElementById(
      "recentHead"
    ).innerHTML = `Your Search result for "${userSearchValueup}" in " ${userSearchSelectionup}" . `;
    console.log(url);
    fetch(url)
      .then((res) => res.json())
      .then((data) => {
        ...
        if(volumeInfo.pageCount == undefined){
          ...
          return ` <div class="row1">
               <div class="row1image"><img src="${thumbnails}" alt="book cover" style="width: 200px; height:200px;"></div>
               <div class="title">${volumeInfo.title}</div>
            </div>
            <div class="row2">
                <div class="head"><p class="allheadings">Author(s):</p> <span>${authors}<span></div>
                <div class="head"><p class="allheadings">Publication Date:</p> ${publishedDate}</div>
                <div class="head"> <p class="allheadings">Rating:</p> ${rating}</div>
                <div class="head"><p class="allheadings">Publisher:</p> ${publishers}</div>
                <div class="head"><p class="allheadings">Number of Pages:</p> ${pageCount}</div>
